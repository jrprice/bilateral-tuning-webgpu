...
function PushShaderConfigToForm(config: ShaderConfig) {
...
const Run = async () => {
  SetRuntime("");
  const iterations = +(<HTMLInputElement>document.getElementById("iterations")).value;
...
const Test = async () => {
  ...
  let num_configs = 0;
  let config: ShaderConfig = <ShaderConfig>{};
...
function GenerateNeighbors(config: ShaderConfig): ShaderConfig[] {
  let neighbors: ShaderConfig[] = [];
...
const Tune = async () => {
  ...
  const iterations = +(<HTMLInputElement>document.getElementById("tune_iterations")).value;

  // Run a config and return the achieved FPS.
  async function Run(config: ShaderConfig) {
  ...
  let current_config: ShaderConfig = GetShaderConfigFromForm();
  let current_fps: number = await Run(current_config);
  SetRuntime(`Initial FPS = ${current_fps.toFixed(1)}`);

  let rounds = 1;
  while (true) {
    let new_best_config: ShaderConfig = null;
    ...
    current_config = { ...new_best_config };
    rounds++;
  }
