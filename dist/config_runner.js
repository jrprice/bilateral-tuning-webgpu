...
class ConfigRunner {
    ...
    constructor(device) {
        ...
        this.sigma_domain = 3.0;
        this.sigma_range = 0.2;
        this.radius = 2;
        this.input_textures = new Array(ConfigRunner.kNumInputImages);
    ...
    async SetupTextures(image) {
        ...
        this.device.queue.copyExternalImageToTexture({ source: image }, { texture: this.input_image_staging }, { width: this.width, height: this.height });
        ...
        for (let i = 0; i < ConfigRunner.kNumInputImages; i++) {
            ...
            commands.copyTextureToTexture({ texture: this.input_image_staging }, { texture: this.input_textures[i] }, { width: this.width, height: this.height });
        }
    ...
    async RunConfig(params) {
        ...
        for (let i = 0; i < ConfigRunner.kNumInputImages; i++) {
            ...
            bind_group_0.push(this.device.createBindGroup({
                entries,
    ...
    GenerateShader(config) {
        ...
        if (config.spatial_coeffs === "inline") {
            if (config.const_sigma_domain) {
                constants += `const kInverseSigmaDomainSquared = ${-0.5 / (this.sigma_domain * this.sigma_domain)};\n`;
        ...
        if (config.const_sigma_range) {
            constants += `const kInverseSigmaRangeSquared = ${-0.5 / (this.sigma_range * this.sigma_range)};\n`;
...
`;
        ...
        if (config.prefetch === "workgroup") {
            ...
            line(`const kPrefetchWidth = kWorkgroupSizeX${config.tilesize_x > 1 ? " * kTileWidth" : ""} + 2*${radius_expr};`);
            line(`const kPrefetchHeight = kWorkgroupSizeY${config.tilesize_y > 1 ? " * kTileHeight" : ""} + 2*${radius_expr};`);
        ...
        if (config.prefetch === "workgroup") {
            line(`let px = i32(lid.x${config.tilesize_x > 1 ? `*kTileWidth + tx` : ""}) + i + ${radius_expr};`);
            line(`let py = i32(lid.y${config.tilesize_y > 1 ? `*kTileHeight + ty` : ""}) + j + ${radius_expr};`);
    ...
    async GenerateReferenceResult() {
        ...
        const commands = this.device.createCommandEncoder();
        commands.copyTextureToBuffer({ texture: this.input_image_staging }, { buffer, bytesPerRow: row_stride * 4 }, {
            width: this.width,
            height: this.height,
        });
        this.device.queue.submit([commands.finish()]);
    ...
    async VerifyResult(params) {
        ...
        const commands = this.device.createCommandEncoder();
        commands.copyTextureToBuffer({ texture: params.output }, { buffer, bytesPerRow: row_stride * 4 }, {
            width: this.width,
            height: this.height,
        });
        this.device.queue.submit([commands.finish()]);
...
ConfigRunner.kNumInputImages = 3;
